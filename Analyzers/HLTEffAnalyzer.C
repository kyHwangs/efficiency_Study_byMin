...
void HLTEffAnalyzer(
  ...
  for(unsigned i_ev=0; i_ev<nEvent; i_ev++){
    ...
    else if( doMem && i_ev !=0 && i_ev % nMem == 0 ) printMemory("\t");
    else printRunTimeShort(timer_total);
    ...
    for(auto& genP: GenParticles){
      ...
      if( genP.get("ID") == 13 && genP.get("isHardProcess") == 1 ) found0 = true;
      if( genP.get("ID") == -13 && genP.get("isHardProcess") == 1 ) found1 = true;
    }
    ...
    if(doDimuon && isDimuon && ZmassWindow > 0.){
      ...
      if(abs(Zmass-91) > ZmassWindow) continue;
    }
    ...
    for(unsigned i=0; i<L3types.size(); ++i){
      ...
      if( !doDimuon || (doDimuon && isDimuon) ){
        if(isDoubleMu){
          if(GenMuonsFromHPInAcc.size() != 2) continue;
          if(GenMuonsFromHPInAcc.at(0).get("charge")*GenMuonsFromHPInAcc.at(1).get("charge") > 0) continue;
          if(!acceptance(GenMuonsFromHPInAcc.at(0))) continue;
          if(!acceptance(GenMuonsFromHPInAcc.at(1))) continue;
          ...
          for(unsigned j=0; j<Eff_genpt_mins.size(); ++j){
            if(genMu0.pt > pt_min_lead && genMu1.pt > Eff_genpt_mins.at(j)){
              ...
              if(passDoubleMu){
                hc_Eff.at(i).at(j)->fill_num( genMu1, nt->truePU, genWeight );
              }
            }
          }
        }
          ...
          for(auto& genmu: GenParticles){
            if( fabs(genmu.get("ID")) != 13 ) continue;
            if( !acceptance( genmu ) ) continue;
            if( genmu.get("status") != 1 ) continue;

            bool fromHardProcess = genmu.matched(GenMuonsFromHardProcess, 0.001);
            if( doDimuon && !fromHardProcess ) continue;
            ...
            if(L3types.at(i).Contains("L1Muon")) matched_idx = -1e6;
            else if(L3types.at(i).Contains("OI") ||
                    L3types.at(i).Contains("L3Muon") ||
                    (std::find(HLTpaths.begin(), HLTpaths.end(), L3types.at(i)) != HLTpaths.end())) matched_idx = genmu.matched( *L3Coll, L3map, 0.1 );
            else matched_idx = looseMatch ? genmu.matched( *L3Coll, L3map, 0.3 ) :     // L2 muon
                                            genmu.matched( *L3Coll, L3map, 0.1, 0.5 ); // IO tracks

            // Mu50OrOldMu100OrTkMu100
            if(L3types.at(i).Contains("Mu50OrOldMu100OrTkMu100") && matched_idx < 0){
            ...
            for(unsigned j=0; j<Eff_genpt_mins.size(); ++j){
              if( genmu.pt > Eff_genpt_mins.at(j) ){
                ...
                if( matched_idx > -1 ) hc_Eff.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );

                if(matched_L1SQ0){
                  hc_Eff_L1SQ0.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                  if(L3types.at(i).Contains("L1Muon")) hc_Eff_L1SQ0.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  else{
                   if( matched_idx > -1 ) hc_Eff_L1SQ0.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  }
                }
                if(matched_L1DQ0){
                  hc_Eff_L1DQ0.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                  if(L3types.at(i).Contains("L1Muon")) hc_Eff_L1DQ0.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  else{
                    if( matched_idx > -1 ) hc_Eff_L1DQ0.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  }
                }
                if(matched_L1SQ8){
                  hc_Eff_L1SQ8.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                  if(L3types.at(i).Contains("L1Muon")) hc_Eff_L1SQ8.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  else{
                    if( matched_idx > -1 ) hc_Eff_L1SQ8.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  }
                }
                if(matched_L1DQ8){
                  hc_Eff_L1DQ8.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                  if(L3types.at(i).Contains("L1Muon")) hc_Eff_L1DQ8.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  else{
                    if( matched_idx > -1 ) hc_Eff_L1DQ8.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  }
                }
                if(matched_L1SQ22){
                  hc_Eff_L1SQ22.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                  if(L3types.at(i).Contains("L1Muon")) hc_Eff_L1SQ22.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  else{
                    if( matched_idx > -1 ) hc_Eff_L1SQ22.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  }
                }
                if(matched_L1DQ22){
                  hc_Eff_L1DQ22.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                  if(L3types.at(i).Contains("L1Muon")) hc_Eff_L1DQ22.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  else{
                    if( matched_idx > -1 ) hc_Eff_L1DQ22.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  }
                }
              }
            }
            ...
            for(unsigned j=0; j<Eff_L3pt_mins.size(); ++j){
              ...
              if( matched_idx > -1 && L3Coll->at(matched_idx).pt > Eff_L3pt_mins.at(j) ) hc_EffTO.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );

              if( matched_L1SQ0 ){
                hc_EffTO_L1SQ0.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                if(L3types.at(i).Contains("L1Muon")){
                  if(genmu.get("l1ptByQ") > Eff_L3pt_mins.at(j)) hc_EffTO_L1SQ0.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
                else{
                  if( matched_idx > -1 && L3Coll->at(matched_idx).pt > Eff_L3pt_mins.at(j) ) hc_EffTO_L1SQ0.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
              }
              if( matched_L1DQ0 ){
                hc_EffTO_L1DQ0.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                if(L3types.at(i).Contains("L1Muon")){
                  if(genmu.get("l1ptByQ") > Eff_L3pt_mins.at(j)) hc_EffTO_L1DQ0.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
                else{
                  if( matched_idx > -1 && L3Coll->at(matched_idx).pt > Eff_L3pt_mins.at(j) ) hc_EffTO_L1DQ0.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
              }
              if( matched_L1SQ8 ){
                hc_EffTO_L1SQ8.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                if(L3types.at(i).Contains("L1Muon")){
                  if(genmu.get("l1ptByQ") > Eff_L3pt_mins.at(j)) hc_EffTO_L1SQ8.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
                else{
                  if( matched_idx > -1 && L3Coll->at(matched_idx).pt > Eff_L3pt_mins.at(j) ) hc_EffTO_L1SQ8.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
              }
              if( matched_L1DQ8 ){
                hc_EffTO_L1DQ8.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                  if(L3types.at(i).Contains("L1Muon")){
                    if(genmu.get("l1ptByQ") > Eff_L3pt_mins.at(j)) hc_EffTO_L1DQ8.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  }
                  else{
                    if( matched_idx > -1 && L3Coll->at(matched_idx).pt > Eff_L3pt_mins.at(j) ) hc_EffTO_L1DQ8.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                  }
              }
              if( matched_L1SQ22 ){
                hc_EffTO_L1SQ22.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                if(L3types.at(i).Contains("L1Muon")){
                  if(genmu.get("l1ptByQ") > Eff_L3pt_mins.at(j)) hc_EffTO_L1SQ22.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
                else{
                  if( matched_idx > -1 && L3Coll->at(matched_idx).pt > Eff_L3pt_mins.at(j) ) hc_EffTO_L1SQ22.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
              }
              if( matched_L1DQ22 ){
                hc_EffTO_L1DQ22.at(i).at(j)->fill_den( genmu, nt->truePU, genWeight );
                if(L3types.at(i).Contains("L1Muon")){
                  if(genmu.get("l1ptByQ") > Eff_L3pt_mins.at(j)) hc_EffTO_L1DQ22.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
                else{
                  if( matched_idx > -1 && L3Coll->at(matched_idx).pt > Eff_L3pt_mins.at(j) ) hc_EffTO_L1DQ22.at(i).at(j)->fill_num( genmu, nt->truePU, genWeight );
                }
              }
            }

            // -- res
            if(matched_idx_res > -1 && i < 18){
