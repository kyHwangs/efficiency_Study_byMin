...
// echo 'gROOT->LoadMacro("drawEffCompIOFromL1wrtL1.C+"); gSystem->Exit(0);' | root -b -l
// rootbq 'drawEffCompIOFromL1wrtL1.C("v01", "DY Winter21", "DYToLL_M50", "L1Tk")'
// rootbq 'drawEffCompIOFromL1wrtL1.C("v01", "DY Winter21", "DYToLL_M50", "")'
...
void drawEffCompIOFromL1wrtL1(
  ...
  vector<TString> files = {
    "./Outputs_"+ver+"/hist-"+ver+"-Run3Base-"+tag+"-Eff"+file_tag+".root",
    "./Outputs_"+ver+"/hist-"+ver+"-wp00-"+tag+"-Eff"+file_tag+".root",
    "./Outputs_"+ver+"/hist-"+ver+"-wp01-"+tag+"-Eff"+file_tag+".root",
    "./Outputs_"+ver+"/hist-"+ver+"-wp00_ROI1p5-"+tag+"-Eff"+file_tag+".root",
    "./Outputs_"+ver+"/hist-"+ver+"-wp01_ROI1p5-"+tag+"-Eff"+file_tag+".root",
    "./Outputs_"+ver+"/hist-"+ver+"-wp00_ROI2p0-"+tag+"-Eff"+file_tag+".root",
    "./Outputs_"+ver+"/hist-"+ver+"-wp01_ROI2p0-"+tag+"-Eff"+file_tag+".root",
    // "./Outputs_"+ver+"/hist-"+ver+"-wp00-"+tag+"-Eff"+file_tag+".root",
    // "./Outputs_"+ver+"/hist-"+ver+"-wp00-"+tag+"-Eff"+file_tag+".root",
    // "./Outputs_"+ver+"/hist-"+ver+"-wp01-"+tag+"-Eff"+file_tag+".root",
  };
  ...
  vector<TString> types = {
    "Eff/num_Eff_"+L1tag+"_hltFromL1Merged",
    "Eff/num_Eff_"+L1tag+"_hltIter0FromL1",
    "Eff/num_Eff_"+L1tag+"_hltIter0FromL1",
    "Eff/num_Eff_"+L1tag+"_hltIter0FromL1",
    "Eff/num_Eff_"+L1tag+"_hltIter0FromL1",
    "Eff/num_Eff_"+L1tag+"_hltIter0FromL1",
  ...
  vector<TString> types_den = {
    "Eff/den_Eff_"+L1tag+"_hltFromL1Merged",
    "Eff/den_Eff_"+L1tag+"_hltIter0FromL1",
    "Eff/den_Eff_"+L1tag+"_hltIter0FromL1",
    "Eff/den_Eff_"+L1tag+"_hltIter0FromL1",
    "Eff/den_Eff_"+L1tag+"_hltIter0FromL1",
    "Eff/den_Eff_"+L1tag+"_hltIter0FromL1",
  ...
  vector<TString> types_str = {
    "IO(L1): Run 3 baseline",
    "IO(L1): IO patatrack",
    "IO(L1): IO patatrack + IO upg.",
    "IO(L1): IO patatrack (ROI#times1.5^{2})",
    "IO(L1): IO patatrack (ROI#times1.5^{2}) + IO upg.",
    "IO(L1): IO patatrack (ROI#times2^{2})",
    "IO(L1): IO patatrack (ROI#times2^{2}) + IO upg.",
  };
  ...
  for(int ipt=0; ipt<(int)v_pts.size(); ++ipt) {
    ...
    for(int ivar=0; ivar<(int)v_var.size(); ++ivar) {
      ...
      if(!v_var[ivar].Contains("pt") || v_var[ivar] == "pt_zoom") {
        ...
        if (true) {  // (!tag.Contains("DYToLL_M50")) {
          ymin = 0.5;
          ymax = 1.35;
        }
      }
      ...
      TLegend *legend;
      SetLegend( legend, 0.15, 0.70, 0.90, 0.83, -1);  // 0.04
